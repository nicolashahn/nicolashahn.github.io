...
<!-- NOTE to self: use textwidth=88 when wrapping -->
...
languages and a high level comparison of the three from a primarily ergonomics-based
perspective._
...
- __Verbosity__: Go is much more verbose (though not Java verbose). Part of that is the
  ...
  and other FP features), not that Python is. If you're comfortable with Python, you can
  go through the [Tour of Go](https://tour.golang.org/welcome/1) in a day or two, and
  you'll have been exposed to the entirety of the language. How I felt at the end of it:
...
- __Interfaces and Structs__: Go uses interfaces and structs where Python would use
  ...
  - However, because Go is statically typed, the compiler (and your IDE's linter) will
    ...
    functionality, but because of the language's dynamicity, it's not foolproof like Go
    is.
...
professional work) is that it's a more "serious business" programming language than
Python is. Due to the fact that the language is small and there aren't many ways to
write the same thing, code ends up looking more uniform throughout a codebase. Combine
this with the static type system, and it really does make for a language that can
comfortably scale to a huge codebase, and developers coming from a variety of other
languages can quickly begin contributing idiomatic and understandable code. If I had to
say what Go would be able to replace, it'd probably be Java, not Python. It's a language
...
A few weeks ago, I decided to give a real try at learning Rust. I had attempted to do so
before but found the type system and borrow checker confusing and without enough context
for why all these constraints were being forced on me, cumbersome for the tasks I was
trying to do. However, since then, I've learned a bit more about what happens with
...
another imperative programming language, they're not too difficult to learn by doing.
